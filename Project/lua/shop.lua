...
function DisplayShop(id,room)
	...
	page=1
	isUse=false
	window1=ui.CreateWindow((768/2)-10,600)
	window2=ui.CreateWindow((768/2)-10,600)
	window1.x,window1.y = display.contentWidth*0.25, 425
	window2.x,window2.y = display.contentWidth*0.75, 425
	swg:insert(window1)
	swg:insert(window2)
	
	shopsign=display.newImageRect("ui/shopsign.png", 128, 64)
...
function BuyMenu()
	...
	for s=1,4 do
		if (curShop.item[s][1]) then
			...
			item[s].prc:setFillColor( 1, 1, 0)
			itemw[s]=display.newRect(0,0,350,80)
			itemw[s].x = item[s].x+135
			itemw[s].y = item[s].y
			itemw[s]:setFillColor(0,0,0,0.1)
			function itemGet()
				ItemInfo(s)
			end
...
function SellMenu()
	...
	if page>1 then
		PrevBtn=  widget.newButton{
			defaultFile="ui/arrow.png",
			overFile="ui/arrow2.png",
			width=50, height=50,
			onRelease = PrevPage
		}
...
function BuyItem(id,name,prc)
	...
	local Full=ui.InvFull()
	local p1=p.GetPlayer()
	if Full==true or prc>p1.gp then
	else
		a.Play(1)
		ui.AddItem(id,stacks,1)
		gp.CallAddCoins(-prc)
	end
end
	...
	if (slot) and type(slot)=="number"  then
		...
		if curShop.item[slot][3]>p1.gp then
			local usebtn=  widget.newButton{
				...
				fontSize=50,
				labelYOffset=10,
				defaultFile="ui/nbutton.png",
				overFile="ui/nbutton-over.png",
				width=200, height=55,
			}
		...
		itemstats=it.ReturnInfo(curShop.item[slot][1])
		...
		if itemstats.type==1 then
			...
			statchange={
				itemstats.stats[1]-equipstats.stats[1],
				itemstats.stats[2]-equipstats.stats[2],
				itemstats.stats[3]-equipstats.stats[3],
				itemstats.stats[4]-equipstats.stats[4],
				itemstats.stats[5]-equipstats.stats[5],
				itemstats.stats[6]-equipstats.stats[6],
				itemstats.armor-equipstats.armor
			}
			stattxts={}
